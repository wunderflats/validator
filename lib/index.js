...
module.exports = function validator (rules, opts) {
  ...
  const CustomError = opts.throwing || false

  if (CustomError && typeof CustomError !== 'function') {
    throw new Error('pass a constructor for throwing validators')
  }

  const flatFilter = (o) => typeof o.validate !== 'function'

  const flatRules = sugarize(flat(rules, {
    filter: flatFilter,
    safe: true
  }))
  ...
  return function validate (input) {
    ...
    if (isAsync) {
      return async
        ...
        .then((errors) => resolve(input, errors))
    }

    return resolve(input, orderedFields.reduce(reducer, {}))
  }

  function resolve (input, errors) {
    if (CustomError) return throwOrResolve(input, errors)
    return Object.keys(errors).length ? flat.unflatten(errors) : null
  }

  function throwOrResolve (input, errors) {
    if (Object.keys(errors).length) {
      throw new CustomError(flat.unflatten(errors))
    }
